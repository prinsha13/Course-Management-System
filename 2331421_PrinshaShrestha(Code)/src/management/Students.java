...
public class Students extends JPanel {
	 ...
	 * Create the panel.
	 */
	public Students(JPanel main, User user) {
		...
		addBtnLbl.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (result == JOptionPane.OK_OPTION) {
					...
					try {
						DatabaseSystem.addStudent(name, number, email, password, Integer.parseInt(courseId),
		...
		editBtnLbl.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (result == JOptionPane.OK_OPTION) {
					...
					String number = numberField.getText().trim();
					try {
						DatabaseSystem.editStudent(id, name, number, email, password);
		...
		removeBtnLbl.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (result == JOptionPane.OK_OPTION) {
					...
					try {
						DatabaseSystem.deleteStudent(studentId);
		...
		if (user instanceof Teacher) {
			JButton markStudentBtn = new JButton("Grade Student");
			markStudentBtn.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					int result = JOptionPane.showConfirmDialog(null, panel, "Grade Student",
					...
					if (result == JOptionPane.OK_OPTION) {
						...
						try {
							DatabaseSystem.gradeStudent(id, moduleId, marks);
							JOptionPane.showMessageDialog(null, "Student graded successfully");
							refreshTable(students);

						} catch (SQLException e1) {
							JOptionPane.showMessageDialog(null, "Error grading student", e1.getMessage(),
									JOptionPane.ERROR_MESSAGE);
						}
					}
				}
	...
	private void createTable(JPanel students) {
		...
		ArrayList<Student> studentData = Retrive.getStudents();
	...
	public static Students getInstance(JPanel main, User user) {
		if (instance == null) {
